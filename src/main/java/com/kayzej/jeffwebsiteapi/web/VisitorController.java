...
public class VisitorController {
    ...
    @Operation(summary = "Store Message", description = "Creates a contact from the provided payload")
    @ApiResponses(value = {
    @ApiResponse(responseCode = "201", description = "Successful creation of message"),
    @ApiResponse(responseCode = "400", description = "Bad request: unsuccessful update", 
        content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
    })
    @PostMapping(value = "/api/recordVisitor", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<Message> createVisitor(@Valid @RequestBody Message
    contact) {
